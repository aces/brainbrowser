...
function BrainBrowser(url) {
   ...
   */
   that.initStep2 = function(clientElements) {
	...
	that.stopDragging(e);
     ...
     //This allows a programmer to define a function that runs after initialiation
     if(that.afterInit) {
       that.afterInit(that);
     }
     that.updateInfo();
  ...
  /*
   * Creates a 2D hud to contain information
   */
  this.createHud = function() {
    that.hudTransform = that.pack.createObject('Transform');
    that.hudViewInfo = o3djs.rendergraph.createBasicView(
      that.pack,
      that.hudTransform,
      that.client.renderGraphRoot);

    that.hudViewInfo.root.priority = that.viewInfo.root.priority + 1;
    that.hudViewInfo.clearBuffer.clearColorFlag = false;

    that.hudViewInfo.drawContext.projection = g_math.matrix4.orthographic(
      0+ 0.5,
      800 + 0.5,
      600 + 0.5,
      0 + 0.5,
    ...
    that.hudViewInfo.drawcontext.view = that.math.matrix4.lookAt(
      [0,0,1],
      [0,0,0],
      [0,1,0]
    );
